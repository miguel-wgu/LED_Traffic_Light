...
Step-by-step wiring instructions for the LED Traffic Light.
...
2. **Connect a 220 ohm resistor** in series with the LED's anode leg. The resistor limits current to a safe level. Without it, the LED draws too much current and can burn out or damage the Arduino pin. One leg of the resistor connects to the LED anode; the other connects to the Arduino digital pin.
...
- The Arduino's digital pins output 5V when HIGH. With a 220 ohm resistor, each LED draws roughly 14mA, which is within the 40mA per-pin limit of the Mega 2560.
- If an LED does not light up, check polarity first. Flip it around if needed.
